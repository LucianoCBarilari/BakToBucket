...
### Added
- **Dockerization**: The application is now fully containerized. A multi-stage `Dockerfile` and a `docker-compose.yml` have been added to simplify deployment.
- **GitHub Container Registry**: The CI/CD pipeline (`deploy.yml`) now automatically builds and publishes the Docker image to `ghcr.io/lucianocbarilari/baktobucket` on every release.

### Changed
- **PostgreSQL Refactoring**: Removed the `docker ps` and `docker exec` dependency from `PostgreSqlBackupProvider`. The application now uses the `Npgsql` package to parse the connection string and connects natively over the network to execute `pg_dump`.
- **Environment Variables**: Completely revamped the `.env.example` to map all `appsettings.json` properties using .NET's double-underscore (`__`) notation, enabling a configuration-as-code deployment via Docker Compose.
